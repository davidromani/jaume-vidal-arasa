...
class Tools
{
    static public function getPercentageFormat($value, $includeSign = true)
    {
        return number_format($value, 0, "'", '.') . ($includeSign ? '%' : '');
    }

    static public function getCoinFormat($value, $includeSign = true)
    {
        return number_format($value, 2, "'", '.') . ($includeSign ? '€' : '');
    }
    ...
    public static function getStringDateFormat(\DateTime $f)
    {
        if ($f == null) {
            return '---';
        } else {
